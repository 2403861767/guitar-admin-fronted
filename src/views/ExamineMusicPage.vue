...
<!--                    <el-button @click="doUpdate(scope.row)" link type="primary" size="large">修改</el-button>-->
                    ...
                    <el-popconfirm @confirm="pass(scope.row.id)" title="确定淘汰吗">
                        <template #reference>
                            <el-button link type="danger" size="large">淘汰</el-button>
                        </template>
                    </el-popconfirm>
                </template>
            </el-table-column>
...
const getMusicList = async () => {
    const res = await myAxios.post('/music/examine', params)
    if (res.code === 0) {
...
// 审核不通过
const pass  = async (id) => {
    const res = await myAxios.get('/music/pass/'+id)
    if (res.code === 0){
        await getMusicList()
